      ...
      <div class="qu-content">
        ...
        >
          ...
          <ul class="operat-list">
            <li v-if="index !== 0" @click="moveUp(index, questionList)">
              上移
            </li>
            <li
              v-if="index !== questionList.length - 1"
              @click="moveDown(index, questionList)"
            >
              下移
            </li>
...
import { apiDomain, getPaper, createPaper, uploadPaper } from "../api";
  ...
  methods: {
    ...
    *releasePaper() {
      yield (() => {
        let nowTime = Date.now();
        let startTime =
          new Date(this.paperObj.startTime.replace(/-/g, ",")) * 1;
        if (startTime > nowTime) {
          this.showPrompt(
            "确认保存并发布问卷吗？(开始日期晚于当前日期，到期才可答题)"
          );
          return;
        }
