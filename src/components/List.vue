  ...
  <div class="container">
    ...
    <div class="list-wrapper" v-else>
      ...
      <ul v-for="paper in paperList">
        <li @click="checkItem(paper)">
          <i :class="{ checked: paper.checked }"></i>
        </li>
        <li>{{ paper.title }}</li>
        <li>{{ paper.endTime }}</li>
        <li :class="paper.status | transformStatusClass">
          {{ paper.status | transformStatusText }}
        </li>
        <li>
          <template v-if="paper.status === 0">
            <span
              @click="
                iterator = editItem(paper);
                iterator.next();
              "
              >编辑</span
            >
